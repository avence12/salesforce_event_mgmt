...
0. **Tag the event** — ★R11 a BD adds free-text **Topic** tags (`Event_Topic__c`, one row per tag, no admin-curated list) as well as picking a type (Symposium / OIP / Conference). Topic is what "similar event" is judged on; type only says what shape it is.
...
| Layer | Items |
|---|---|
| Objects | `Marketing_Event__c` (+3 roll-up counters), `Event_Attendee__c` (the imported person; free-text `Company__c`, `Unique_Key__c` for de-duplication), `Event_Invitee__c` (junction: event × attendee, unique per pair, `Attended__c`, and formulas reading both down to the attendee and up to the event), ★R11 `Event_Topic__c` (one free-text tag per row, BD-maintained, for similarity) |
...
| Declarative | **Approval Process** `Invitee_Approval` — ★R9 **five steps**, each addressed to its own `Approver_N__c` and skipped when that level is blank — **3 reports** + 3 custom report types (★R11 adds `Marketing_Events_with_Topics`), record-triggered Flows `Invitee_Decision_Completion` and ★R9 `Invitee_Level_Advanced`, 3 validation rules |
...
- ★R9 **A reporting line, and customers with owners.** Routing starts at the Account Owner of the
  ...
  the attendee is linked to a Contact (`Event_Attendee__c.Contact__c`, ★R12 set by the import),
...
3. **★R9 Set up the approval chain** (Setup → Users, and your Account data). Routing starts at the Account Owner, so: give each demo Account an owner, set `ManagerId` up the reporting line above them, and put the regional-head marker in the `Title` of whoever ends the chain (the marker itself is `Approval_Chain_Setting__mdt` → *Default* → *Regional Head Titles*, shipped as `Regional Head`; the level cap is beside it, shipped as 2). The BMD user's own Manager is no longer used by routing.
   *A guest who belongs to no customer cannot be submitted at all* — see step 8.
...
8. **★R9 Create the non-customer-guest Account, and reconcile the guests behind it** (Open Question 21). Professors, press and anyone else who is nobody's customer have no Account Owner and therefore no approval chain, so the submit refuses them by design. **This repo does not and will not do this for you:** no code here writes to Account, Contact or Lead, which is the invariant the tests assert. The full manual runbook is [DEPLOYMENT.md, post-deploy step 10](DEPLOYMENT.md#step-5-post-deploy-setup-10-minutes) — in short: create an Account named so no report mistakes it for a real customer, set its owner as level 1, create a Contact under it per guest, and link each guest's `Event_Attendee__c.Contact__c` by hand. **For the demo, leave `Hélène Dubois` unlinked** — she is the seed data's example of a guest with no chain, and Success Criterion 7 / demo script step 3b depend on her submit being refused.
...
OWD check. The workflow reads none of those objects, so it asks nothing of them. ★R11 also
removed a step that used to be here: seeding a real `Topic__c` taxonomy before tagging meant
anything. There is no taxonomy to seed any more — a BD tags an event from its own record page,
the moment they think of it.
...
- **Similarity is judged on Topic tags (`Event_Topic__c`), not `Event_Type__c`.** Three type values say what shape an event is; recommending from them would just mean "another conference for conference-goers". ★R11 Topic is free text a BD adds, not a picklist, so it carries the same "no dedup, no roll-up" cost `Company__c` already does — accepted for the same reason.
...
- **★R10 8000-row import cap, chunked into 200-row Apex calls** — a computed SOQL query-length budget put the derived safe size at 100; 200 ships instead by business decision, at roughly half that margin (see design.md → ★R10). The single-transaction 500-row cap is gone; a whole file is parsed and de-duplicated client-side, then sent to Apex in sequential batches so no one Apex call carries anywhere near a whole large file. `applyChanges` now tolerates a bad row within a batch (`Database.upsert(..., false)`) rather than discarding the batch, and the result screen reports created / updated / failed with a downloadable failure list. What that costs: batches are independent, so a mid-import failure can leave the file only partly applied — the remedy is fix-and-re-upload, which is safe because the upsert is idempotent, not a rollback. See [design.md → ★R10](design.md#r10-8000-row-import-chunking-and-partial-failure).
...
- **★R9 A guest with no customer cannot be submitted.** Routing needs an Account Owner. Post-deploy step 8 is the remedy and it is a data one; until it is done, non-customer guests can be added to an event and go no further.
...
- **★R12 The import links an attendee to a Contact, on customer code + email.** `Event_Attendee__c.Contact__c` records that an imported person is also a Contact, and `Is_Known_Contact__c` makes "which of tonight's guests are already known to us?" reportable. The match needs the CSV's `Cust Cd` **and** `Email` to agree with one Contact's `Account.AccountNumber` and `Email`; no hit and several hits both mean no link, so a blank answer is "not established" rather than "no". This reverses R8, which forbade the import to touch Contact and relied on a reconciliation step that was never built. The import reads Contact and Account; it writes neither, ever.
...
- **★R11 Topic tags have no de-duplication.** "AI & Data" and "ai and data" are two tags, not one, and no report rolls them up — the same accepted cost as `Company__c`. Historical events still need a human to tag them before they contribute to any recommendation; R11 removed the admin-taxonomy blocker, not the back-filling work itself.
