...
1. **Import** — AM uploads a CSV (.csv) contact list (comma, semicolon or tab separated; must be UTF-8); rows are matched by email with a diff preview (New contact / **New lead** / Update / Unchanged / Company change / Skipped) before anything is applied. A row whose company is not an existing customer Account becomes a **Lead** — no Account is ever created by an import.
...
| Layer | Items |
|---|---|
| Objects | `Marketing_Event__c` (+3 roll-up counters), `Event_Invitee__c` (junction onto **either** a Contact or a Lead, unique per event+invitee, 5 `Invitee_*__c` formula fields that read correctly for both) |
...
7. **Seed demo data**: edit the two owner usernames at the top of `scripts/seed-demo-data.apex`, then
   `sf apex run --file scripts/seed-demo-data.apex -o poc-sandbox`
8. Demo import file: `demo-data/FinTech_Summit_2026_Attendees.csv` (regenerate with `node scripts/generate-demo-csv.mjs`).

## Demo script (5 minutes)

1. As **AM**: open the *Event Management* app → *Import Contacts* tab → upload the demo .csv → show the diff preview (incl. the new-lead rows and the company-change manual list) → Apply.
