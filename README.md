...
3. **Approve** — Each BMD user submits their batch into the **standard Approval Process**, routed to their **manager**; each approver gets one aggregated email + bell notification and decides from **★R8 Approvals by Company** on the event page — their pending invitees grouped by the company each was invited as, so one tick selects a whole company and one button decides it, with per-person veto still available. The standard Approvals list and the Salesforce Mobile App still work as before; what the component adds is the grouping, which the standard list cannot do. Full approval history on every invitee either way.
...
3. As the **approving manager** (desktop or the Salesforce Mobile App): open the bell notification, then the event's **Approvals by Company** card. Tick one company → *Approve 3*. Then untick one person in the next company and approve the rest, to show that per-company bulk and per-person veto are the same mechanism. Show the **Approval History** on a decided invitee, and a guest marked *Not a customer* — nobody's Contact, so no account and no customer code behind them.
